...
function A_star(s: Vertex, t: Vertex, G: Graph): [number[], number[]] {
    let [distances, parents] = initialization(s, G);
    let GRIS = new Set([s]);
    let NOIR = new Set<Vertex>();
    ...
    while (true) {
      ...
      let x = calculate_d_plus_h_min(GRIS, distances, G);
      if (x == t) {
        return [distances, parents];
      }
      let L = get_outgoing_arcs(x, G);
...
function initialization(s: Vertex, G: Graph): [number[], number[]] {
    let d : number[] = new Array(G.size);
    let parent : number[] = new Array(G.size);
